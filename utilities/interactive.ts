...
export function handleSlut (state: GameState): GameResults
{
    const newState: GameState = {...state};

    const cooldownError: GameResults | null = onCooldown(state, state.cooldownSlut);
    if (cooldownError !== null) return cooldownError;

    const slutPrize: number = Math.floor(Math.random() * (300 - 100 + 1)) + 100;

    newState.cash += slutPrize;
    newState.cooldownSlut = Date.now() + 120000;

    const successResults: GameResults = {
        newState: calculateTotal(newState),
        message: `You slutted hard and earned ${slutPrize}`,
        success: true
    };
